...
- [x] AIモデル2資産は右サイドの明示操作でだけ取得を開始する
...
- [x] 初回取得・破損時を1つの「AIモデルを取得」に統一し、正常時の再取得操作は表示しない。右サイド「アプリ」タブの「Stem分離」へ統合し、手動の「モデル検証」と「選択できる操作」の見出しは置かない
- [x] 取得元・Revision・License・2資産の容量・SHA-256・固定URL・保存先をアプリメニューの「Veloura Lucentについて」へ移動
- [x] 取得中・失敗・保存先・標準インジケーター・完了チェック・Liquid GlassキャンセルボタンをRootの画面内Liquid Glassモーダルへ移し、割合・受信容量・ファイル別進捗バーと右サイドの重複表示を削除
...
- [x] 2026-07-30、Release実画面で右サイド「Stem分離」のHTDemucs／BS-RoFormer-SW切替、両モデルの利用可能状態、当時の「AIモデルを取得」と「モデル検証」の2操作、Aboutの配布情報を確認。Rootの画面内Liquid Glassモーダルでは、保存先、取得ファイル、標準インジケーター、通常時が赤文字のキャンセル、割合・受信容量・ファイル別進捗バーがないこと、初期表示でキャンセルに青いフォーカス枠が付かないこと、取得成功後に自動で閉じて利用可能へ戻ることを確認。透明Glassの見た目を維持し、幅だけ660ptへ固定した。キャンセルの赤いGlassホバーと白文字はコード構造・Focused Testまで確認し、実ポインタによる実画面確認は未完了。2026-07-31に「モデル検証」と「選択できる操作」の見出しを削除した後の実画面は未確認
- [ ] 実画面で入力選択、補正、再ミックス、マスタリング、書き出しを確認
...
- 2026-07-24、専用モデル管理画面を廃止し、右サイド「アプリ」タブの「Stem分離」へモデル状態と取得を統合した。2026-07-30、初回・破損時・完全再取得の表示を1つの「AIモデルを取得」へ統一した。2026-07-31、手動の「モデル検証」と「選択できる操作」の見出しを削除し、起動時・モデル切替時の自動検証だけを残した。2026-08-03、正常時の完全再取得操作を削除し、初回・破損時だけ取得操作を表示するよう変更した。
- 2026-07-30、配布情報はアプリメニューの「Veloura Lucentについて」へ移した。取得中はRootの画面内Liquid Glassモーダルに保存先、取得ファイル、標準インジケーター、Liquid Glassキャンセルボタンを表示し、完了時はチェック、失敗時は理由を同じモーダルへ表示する。割合、受信容量、ファイル別進捗バーは表示しない。2026-07-31、ダウンロード後のモデル本体検証を有効化直前の1回へ集約し、最終検証で異常を検知した時だけ同じモーダルへ「再ダウンロード」を表示するようにした。
...
- モデル検証・取得・有効化の中核処理は変更せず、右サイドの明示操作を通信開始の契機にした。
