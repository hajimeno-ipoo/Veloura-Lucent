...
- [x] 補正段とマスタリング段を別のユーザー操作に分離した
- [x] 作業状態は通常モードと同じくアプリ起動中のメモリで保持する
- [x] Stem一時音声はsystem temporary directory配下に限定した
- [x] モデル管理と音楽処理を分離した
- [x] AIの責務は4Stem分離で終了する
- [x] 補正後は保存済み4Stemと補正後再ミックスを表示して停止し、マスタリングを自動実行しない
- [x] 補正完了率100%、正常な補正済み4Stem、検証済み補正後再ミックスが揃うまでマスタリングを開始できない
...
- [x] 補正後再ミックスを48 kHz、stereo、Float32で一時保存して検証
- [x] 補正完了直後に補正後再ミックスの波形、スペクトログラム、解析値、試聴を接続
- [x] 正常な補正済み4Stemと検証済み補正後再ミックスが揃った時だけ補正完了と100%にする
- [x] 補正完了後はマスタリング待機で停止
...
- [x] 補正段で保存・検証・表示した同一の補正後再ミックスを、再生成や重複WAVコピーなしで既存`MasteringService`へ直接渡す
- [x] 既存マスタリング前解析、トーン、de-ess、ダイナミクス、ステレオ、ラウドネス、ピーク、最終guardを維持
- [x] Stem Mode最終版を一時WAVとして保存
- [x] 通常モード共通の最終解析・品質レポートを生成
- [x] ユーザー操作で補正後再ミックス、補正済みStem、最終版を書き出す

## 6. 一時保存とキャンセル

- [x] 保持するStem一時音声を分離入力、raw 4Stem、補正済み4Stem、補正後再ミックス、最終版に限定
...
- [x] マスタリングキャンセル／失敗時は補正済み4Stem、補正後再ミックス、補正完了表示を維持し、最終版の未完成音声だけを削除
...
- [x] 上部の入力、補正、マスタリング、書き出しを通常モードと同じ操作順にする
...
- [x] メニューバーの音声選択、補正、マスタリング、2mix再生、書き出しをRoot所有の現在モード操作へ接続する
- [x] 表示メニューの「モード」から通常補正／Stem Modeを選び、アプリ本体を既存`selectMode`で切り替える
- [x] Stem Modeのメニューバー書き出しに、補正後再ミックス、補正済み4Stem、Stem Mode最終版を表示し、未生成項目だけを無効化する
...
- [x] 詳細解析へ入力2mix／raw再ミックス／補正後再ミックスの既存検証測定値を接続する
...
- [x] 左の音源区画と工程区画を維持
- [x] 補正とマスタリングの状態、現在工程、詳細、進捗率、実際の全内部工程を別ブロックで表示
- [x] 中央の入力、補正後再ミックス、最終版の波形・解析・試聴を接続
...
- [x] 右上の補正、マスタリング、アプリタブの役割を通常モードと同じにする
...
- [x] 右下の「解析結果と品質確認」は、選択中の入力／補正後／最終版についてIntegrated Loudness、True Peak、ダイナミクス、ステレオ幅、品質警告、完了後レポートを表示し、未解析時は小型状態表示にする
- [x] 直近ログを入力、補正、マスタリング、ユーザー書き出しの構造化操作履歴にする
- [x] 全体進捗の書き出しを、内部最終版生成ではなくユーザーによる外部保存として扱う
- [x] 詳細ログは補正とマスタリングを分け、制御イベントを除き、入力準備、分離、検証、各Stem解析・NoiseMeasurement・route・DSP・guard・保存、再ミックス、最終版保存・解析までの主要工程を短い行で実行順に表示する
...
- [ ] 実画面で右サイド「Stem分離」のモデル状態・モデル検証・モデル再取得、入力選択、補正、マスタリング、書き出しを確認
- [ ] 実画面で補正キャンセルとマスタリングキャンセルを確認
