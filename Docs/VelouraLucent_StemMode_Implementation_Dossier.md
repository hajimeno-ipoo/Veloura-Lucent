...
| `Sources/VelouraLucent/Views/StemModeControlsView.swift` | 新規または既存View拡張 | モード、ステム出力、進捗のUI |
...
現行の `outputFile` は、Stem Modeでは「再ミックス後・マスタリング前の2mix」を指す値として使う。

この扱いにより、既存の比較・プレビュー・マスタリング後分析の多くを再利用できる。

ただし、現行の `beginProcessing(...)`、`finishSuccess(...)`、`finishFailure(...)` は通常補正用の文言、進捗、通知、最近の操作を更新する。

Stem Modeではこれらをそのまま呼ばず、Stem Mode専用の開始・完了・失敗メソッドを追加する。

そのうえで、再ミックス後の2mixを既存UIの「補正後」に相当する表示へ接続する場合だけ、`outputFile`、`outputFileInfo`、`outputMasteringAnalysis`、`outputNoiseMeasurements` を更新する。
...
|---|---|---|
| 通常補正 | `補正を実行` | 現行どおり |
| Stem Mode | `Stem Modeを実行` | 分離→補正→再ミックス→マスタリング |
| 通常補正中 | Stem Modeを無効化 | 同時実行禁止 |
| Stem Mode中 | 通常補正／マスタリングを無効化 | 同時実行禁止 |

Stem Modeの完了後に表示する成果物:
...
- MLXはApple Silicon前提のため、Intel Mac対応をどう扱うか決める必要がある
...
### フェーズ2: Stem Modeの状態・UI

1. `ProcessingMode` を追加
2. `ProcessingJob` にStem Mode状態を追加
3. Stem Mode専用進捗を追加
4. Stem Mode専用の最近の操作ドメインを追加
5. Stem Mode完了通知の扱いを追加
6. 再ミックスを `outputFile` として既存表示に接続
7. Stem出力の表示・書き出しを追加
...
1. モデル採用資料を一次情報と実測結果で完成
2. MLX Swift依存を追加
3. `htdemucs.safetensors`、`htdemucs_config.json`、`mlx.metallib` の配置を決定
4. `StemModelContract` を定義
5. `MLXStemSeparationService` を実装
6. PyTorch版、Python MLX版、MLX Swift版の差分測定を記録する
7. macOS実機でロード・推論・再ミックスを検証

完了条件:

```text
選定モデルの入力・出力・ライセンス・実行方式・重み取得手順・成果物が
リポジトリ内の資料とテストで追跡できる
```
...
以下は確認時に取得したファイル内容の識別子である。リンクは `master` ブランチを指すため、将来の変更後に参照する場合は、各ファイルのblob SHAも併せて確認する。

| ファイル | 確認したblob SHA | 主な根拠 |
|---|---|---|
| `Package.swift` | `e4de5fa9e0c0df913011316ecc582bffc0e061b8` | macOS 26、リソース登録 |
| `Services/AudioFileService.swift` | `ba2d27669778a09b3c2f8c39911d681a7d9ce8e3` | Float32読込、48 kHz変換、保存 |
| `Services/AudioProcessingService.swift` | `3c4e6645495410674fcff1ad545de74dcb483b61` | 補正サービスのURL入出力 |
| `Services/NativeAudioProcessor.swift` | `eda30c799767007dc508db2671914c8def91cce2` | 補正工程 |
| `Services/MasteringService.swift` | `1cdfa2c5945a900f45199709632e0c90ed3c87aa` | 原音参照、マスタリング経路 |
| `Services/MasteringProcessor.swift` | `7bbd6e39c5c25dab05933bf9e2d831b944f7986c` | マスタリング工程 |
| `Models/ProcessingJob.swift` | `2421b274a04a6d93bff4f7be75a1917357f35b0f` | 現行状態管理 |
| `Views/ContentView.swift` | `492b0daa679959631e6864334c88dd1ca254762a` | 現行UI操作 |
| `Tests/VelouraLucentTests/MasteringPipelineTests.swift` | `30d561507f337d529c0b4867fd898f680d3fed48` | 現行マスタリングテスト |
