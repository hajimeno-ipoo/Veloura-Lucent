...
次に進めるなら、実装ではなく、まず `AudioAnalysisMode` と `MetalAudioAnalysisProcessor` の空実装を追加します。

この段階では音声処理結果を変えず、CPUフォールバックだけを通します。その後に、`separatedMeanSpectra` のGPU試作へ進むのが安全です。
