...
struct UIWordingPolicyTests {
    ...
    func appBundleDeclaresJapaneseAsItsLocalization() throws {
        ...
        #expect(buildScript.contains("mktemp -d \"$DIST_DIR/.veloura-lucent-stage.XXXXXX\""))
        #expect(buildScript.contains("mktemp -d \"$DIST_DIR/.veloura-lucent-backup.XXXXXX\""))
        #expect(!buildScript.contains("DEVELOPMENT_TEMP_ROOT="))
