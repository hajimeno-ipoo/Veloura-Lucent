...
struct UIWordingPolicyTests {
    ...
    func appBundleDeclaresJapaneseAsItsLocalization() throws {
        ...
        #expect(buildScript.contains("DEVELOPMENT_TEMP_ROOT=\"${TMPDIR:-/private/tmp}\""))
        #expect(buildScript.contains("DEVELOPMENT_OUTPUT_DIR=\"${DEVELOPMENT_TEMP_ROOT%/}/VelouraLucentDevelopment\""))
        #expect(buildScript.contains("OUTPUT_DIR=\"$DIST_DIR\""))
        #expect(buildScript.contains("OUTPUT_DIR=\"$DEVELOPMENT_OUTPUT_DIR\""))
        #expect(buildScript.contains("FINAL_APP_BUNDLE=\"$OUTPUT_DIR/$DISPLAY_NAME.app\""))
        #expect(buildScript.contains("mktemp -d \"$OUTPUT_DIR/.veloura-lucent-stage.XXXXXX\""))
        #expect(buildScript.contains("mktemp -d \"$OUTPUT_DIR/.veloura-lucent-backup.XXXXXX\""))
        #expect(!buildScript.contains("FINAL_APP_BUNDLE=\"$DIST_DIR/$DISPLAY_NAME.app\""))
