...
| 試聴比較 | 入力、補正後、最終版を波形で確認し、A/B 再生で聴き比べます。Stem Modeでは、選択中Stemの分離直後と補正後も独立した波形欄で確認できます |
...
- `基本表示`: 波形と試聴比較、Stem Modeの分離直後／補正後Stem比較、平均スペクトル、ベクトルスコープ、ラウドネスメーター、スペクトログラム
- `詳細解析`: 主要数値比較、ノイズ7種類比較、ステレオ相関、短時間ラウドネス、ダイナミクス推移、平均スペクトル比較、周波数帯域詳細

右サイドは `補正`、`マスタリング`、`アプリ` に分かれています。

- `補正`: 補正プリセット、基本、掃除と修復、上級
- `マスタリング`: 仕上がりプロファイル、基本、音色、上級
- `アプリ`: 背景の透明感、完了通知、解析モード

3つの設定タブの下に独立した`解析結果と品質確認`があり、入力、補正後、最終版の主要測定値、品質警告、完了後レポートを表示します。
...
| 表示 | 内容 |
| --- | --- |
| 波形と試聴比較 | 入力、補正後、最終版の波形と再生位置を表示します。Stem Modeでは、選択中の分離直後Stemと補正後Stemを別の操作欄で上下表示します |
