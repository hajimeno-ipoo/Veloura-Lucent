...
enum ModelLoader {
    ...
    typealias LocalCandidateProvider = (_ modelName: String, _ preferred: URL?) -> [URL]
    typealias HubDirectoryDownloader = (_ modelName: String, _ repoID: String) throws -> URL

    /// Resolve the directory containing model files (safetensors + config JSON).
    static func resolveModelDirectory(
        modelName: String,
        preferred: URL?,
        policy: DemucsModelResolutionPolicy = .localThenHub,
        candidateProvider: LocalCandidateProvider = localThenHubCandidateDirectories,
        hubDownload: HubDirectoryDownloader = downloadModelDirectoryFromHub
    ) throws -> URL {
        ...
        if policy == .localOnly {
            guard let preferred else {
                throw DemucsError.unsupportedModelBackend(
                    "Local-only model resolution requires an explicit modelDirectory for '\(modelName)'."
                )
            }

            let missing = missingRequiredModelFiles(
                in: preferred,
                modelName: modelName,
                fileManager: fm
            )
            guard missing.isEmpty else {
                throw DemucsError.unsupportedModelBackend(
                    "Local-only model directory '\(preferred.path)' is missing required file(s): "
        ...
        let candidates = candidateProvider(modelName, preferred)
        ...
        do {
            return try hubDownload(modelName, resolvedRepo)
        } catch {
    ...
    /// Build the complete local search order used by the compatibility `.localThenHub` policy.
    static func localThenHubCandidateDirectories(modelName: String, preferred: URL?) -> [URL] {
        let fm = FileManager.default
        var candidates: [URL] = []
    ...
    /// Return the exact filenames required for a model that are absent from a directory.
    static func missingRequiredModelFiles(
        in directory: URL,
        modelName: String,
        fileManager: FileManager = .default
    ) -> [String] {
        let weightsFilename = "\(modelName).safetensors"
        let configFilename = "\(modelName)_config.json"
        ...
        if !fileManager.fileExists(atPath: weights.path) {
            missing.append(weightsFilename)
        }
        if !fileManager.fileExists(atPath: config.path) {
            missing.append(configFilename)
        }
        return missing
    }
