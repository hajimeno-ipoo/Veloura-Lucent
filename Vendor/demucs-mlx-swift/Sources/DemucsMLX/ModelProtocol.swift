...
enum DemucsModelFactory {
    ...
    private static func makeBagOfModels(
        ...
        config: [String: Any],
        directory: URL,
        modelResolutionPolicy: DemucsModelResolutionPolicy
    ) throws -> StemSeparationModel {
    ...
    struct QuantizationConfiguration: Equatable {
        let bits: Int
        let groupSize: Int
    }

    /// Resolve environment-driven quantization without mutating process-global state.
    /// Veloura Lucent's strict local-only path must load the validated weights unchanged.
    static func quantizationConfiguration(
        environment: [String: String],
        modelResolutionPolicy: DemucsModelResolutionPolicy
    ) -> QuantizationConfiguration? {
        guard modelResolutionPolicy != .localOnly,
              let bitsString = environment["DEMUCS_QUANTIZE_BITS"],
              let bits = Int(bitsString),
              [4, 8].contains(bits) else {
            return nil
        }
        let groupSize = Int(environment["DEMUCS_QUANTIZE_GROUP_SIZE"] ?? "64") ?? 64
        return QuantizationConfiguration(bits: bits, groupSize: groupSize)
    }

    /// Apply post-load quantization to a model graph's Linear layers.
    private static func applyQuantizationIfRequested(
        _ graph: Module,
        modelResolutionPolicy: DemucsModelResolutionPolicy
    ) {
        guard let configuration = quantizationConfiguration(
    ...
    private static func buildModelGraph(
        ...
        case "HTDemucsMLX", "HTDemucs":
            ...
            applyQuantizationIfRequested(graph, modelResolutionPolicy: modelResolutionPolicy)
            MLX.eval(graph.parameters())
