...
final class DemucsGraph: Module {
    ...
    // MARK: - Resampling (Metal GPU kernel)

    /// Upsample by 2 using fused Metal kernel (zero-insertion + reflect-pad + FIR).
    private func resample2x(_ x: MLXArray) -> MLXArray {
        metalResample2x(x)
    }

    /// Downsample by 2 using fused Metal kernel (reflect-pad + FIR + decimation).
